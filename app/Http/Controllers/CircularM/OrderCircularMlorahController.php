...
class OrderCircularMlorahController extends Controller
{
        ...
        switch ($form_name) {
            ...
            case 'formKegiatanMesin':
                ...
                $filtered = array_values(array_filter($list_type_mesin, function ($item) {
                    return in_array($item->IdType_Mesin, ['21']);
                }));
                // $filtered = array_values(array_filter($list_type_mesin, function ($item) {
            ...
            default:
                return view('CircularM.transaksi.' . $form_name, compact('access'));
        }

        return view('CircularM.transaksi.' . $form_name, $form_data, compact('access'));
    }
    ...
    public function show(Request $request, $id)
    {
        if ($id == 'getTanggal') {
            ...
            foreach ($results as $row) {
                ...
                if (in_array(trim($row->IdType_Mesin), ['21'])) {
                    $response[] = [
            ...
            $results = DB::connection('ConnCircular')
                ->select('EXEC SP_4451_KegiatanMesin @Kode = ?, @tgl_awal = ?, @tgl_akhir = ?, @shift = ?', [3, $tgl_awal, $tgl_akhir, $request->input('shift')]);
            // dd($results);
            $response = [];
            foreach ($results as $row) {
                $response[] = [
