...
class KonversiBalikLamiController extends Controller
{
    public function index()
    {
        $nomorUser = trim(Auth::user()->NomorUser);
        $access = (new HakAksesController)->HakAksesFiturMaster('ABM');
        return view('ABM.Konversi.BalikLami.KonversiBalikLamiABM', compact('access', 'nomorUser'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $Tgl_konversiBL = $request->input('Tgl_konversiBL');
        $shiftBL = $request->input('shiftBL');
        $idMesinBL = $request->input('idMesinBL');
        $hasilPCSBL = (float) $request->input('hasilPCSBL');
        $hasilKgBL = (float) $request->input('hasilKgBL');
        ...
        $dataTujuanKonversi = $request->input('dataTujuanKonversi');
        $idOrderKerja = $request->input('idOrderKerja');
        $nomorOrderKerja = $request->input('nomorOrderKerja');
        $uraian_asal = (string) 'Group ' . $shiftBL . ", Asal Konversi ABM Balik Lami | Nomor Order Kerja: " . $nomorOrderKerja;
        $uraian_tujuan = (string) 'Group ' . $shiftBL . ", Tujuan Konversi ABM Balik Lami | Nomor Order Kerja: " . $nomorOrderKerja;
        ...
        foreach ($dataAsalKonversi as $row) {
            ...
            $result = DB::connection('ConnABM')
                ->select(
                    'EXEC SP_4384_ABM_Konversi_Balik_Lami @XKode = ?, @XKodeBarang = ?, @XNomorIndeks = ?',
        ...
        try {
            ...
            $currentIdKonvBalikLamiABM = DB::connection('ConnInventory')
                ->table('Counter')->value('IdKonvBalikLamiABM');
            $newIdKonvBalikLamiABM = $currentIdKonvBalikLamiABM + 1;
            DB::connection('ConnInventory')
                ->table('Counter')->update(['IdKonvBalikLamiABM' => $newIdKonvBalikLamiABM]);
            $idkonversi = "ABL" . str_pad($newIdKonvBalikLamiABM, 6, "0", STR_PAD_LEFT);
            ...
            for ($i = 0; $i < count($dataAsalKonversi); $i++) {
                ...
                DB::connection('ConnABM')
                    ->statement('EXEC SP_4384_ABM_Konversi_Balik_Lami
                        @XKode = ?,
            ...
            return response()->json(['error' => (string) "Terjadi Kesalahan code: 1! " . $ex->getMessage(), 'errorDetail' => $ex]);
        }
        ...
        try {
            ...
            $DataTypeTujuan = DB::connection('ConnABM')
                ->select(
                    'EXEC SP_4384_ABM_Konversi_Balik_Lami @XKode = ?, @XKodeBarang = ?, @XIdMesin = ?, @XIdOrder = ?',
                    [5, $kodeBarangHasil, $idMesinBL, $idOrderKerja]
                );
            ...
            // belum ada keterangan tentang sisa barcode dari balik lami
            foreach ($dataAsalKonversi as $row) {
                ...
                if ($sisaTritier !== (float) 0 && $sisaSekunder !== (float) 0 && $sisaBarcodeAsalManual) {
                    ...
                    $JumlahMasukSekunder = [$hasilPCSBL, $jumlahSekunderBarcodeSisa];
                    $JumlahMasukTritier = [$hasilKgBL, $jumlahTritierBarcodeSisa];
                    $idSubkelompokTujuan = [$DataTypeTujuan[0]->IdSubkelompok, $row[13]];
                    $uraian_tujuanKonversi = [
                        $uraian_tujuan,
                        (string) 'Group ' . $shiftBL . ', Sisa Konversi ABM Printing | Nomor Order Kerja: ' . $nomorOrderKerja . ' | Sisa Persen: ' . $sisaPersen
                    ];
                } else {
                    // $jumlahHasilKonversi = 1;
                    $IdTypeTujuan = [$DataTypeTujuan[0]->IdType];
                    $JumlahMasukSekunder = [$hasilPCSBL];
                    $JumlahMasukTritier = [$hasilKgBL];
                    $idSubkelompokTujuan = [$DataTypeTujuan[0]->IdSubkelompok];
                    $uraian_tujuanKonversi = [$uraian_tujuan];
                }
            }
            for ($k = 0; $k < count($dataTujuanKonversi); $k++) {
                DB::connection('ConnABM')
                    ->statement('EXEC SP_4384_ABM_Konversi_Balik_Lami
                            @XKode = ?,
                            @XUraianDetailTransaksi = ?,
                        ...
                        [
                            ...
                            $date,
                            1,
                            $dataTujuanKonversi[$k][2],
                            $dataTujuanKonversi[$k][3],
                            $idSubkelompokTujuan[0],
                            $idkonversi,
                            $dateTime,
                            0,
                        ]
            ...
            // ACC Konversi
            DB::connection('ConnABM')
                ->statement('EXEC SP_4384_ABM_Konversi_Balik_Lami @XKode = ?, @XIdKonversi = ?, @XKdUser = ?', [6, $idkonversi, $nomorUser]);

            // SELECT BARCODE AFTER ACC
            $barcodeHasil = DB::connection('ConnABM')
                ->select('EXEC SP_4384_ABM_Konversi_Balik_Lami @XKode = ?, @XIdKonversi = ?', [7, $idkonversi]);
            ...
            return response()->json(['error' => (string) "Terjadi Kesalahan!  code: 2 " . $e->getMessage(), 'errorDetail' => $e]);
        }
    }

    public function show($id, Request $request)
    {
        if ($id == 'getBarcodeAktif') {
            try {
                $dataBarcode = DB::connection('ConnABM')
                    ->select('EXEC SP_4384_ABM_Konversi_Balik_Lami @XKode = ?', [8]);

                return datatables($dataBarcode)->make(true);
            } catch (Exception $e) {
                return response()->json(['error' => (string) "Terjadi Kesalahan! " . $e->getMessage()]);
            }
        } else if ($id == 'getDataKonversiAsalBalikLami') {
            ...
            try {
                $dataMesin = DB::connection('ConnABM')->select('EXEC SP_4384_ABM_Konversi_Balik_Lami @XKode = ?', [4]);
