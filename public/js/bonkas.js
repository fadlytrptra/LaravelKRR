// list bonkas (putih + merah)
let modeKirim = '';
$(function () {
    console.log($.fn.DataTable.isDataTable('#tblBonKas'));
    ...
    $.fn.dataTable.ext.search.push(function (settings, data, dataIndex) {
        ...
        // FILTER STATUS
        if (filterStatus && String(row.Status) !== filterStatus) {
            return false;
        }
    ...
    $('#filterTanggalAwal, #filterTanggalAkhir, #filterJenisBonKas, #filterStatus').on('change', function () {
        table.draw();
    });
    ...
    $('#btnProsesKirim').on('click', function () {
        console.count('btnProsesKirim CLICK');
        ...
        // =====================================================
        // FORM BARU (BELUM PERNAH DISIMPAN)
        // =====================================================
        if (!idBonKas) {
            // =========================
            // BON KAS PUTIH
            // =========================
            if (modeKirim === 'PUTIH') {
                const $form = $('#formBonKasPutih');

                $form.find('[name="action"]').val('kirim');
                $form.find('[name="Mengetahui"]').val(nomorUser);

                let jumlah = $('#modalTambahBonKasPutih #jumlah')
                    .val()
                    .replace(/,/g, '');

                $('#modalTambahBonKasPutih #jumlah').val(jumlah);
            ...
            if (modeKirim === 'MERAH') {
                ...
                const $form = $('#formBonKasMerah');

                $form.find('[name="action"]').val('kirim');
                $form.find('[name="Mengetahui"]').val(nomorUser);

                let jumlah = $('#modalTambahBonKasMerah #jumlah')
                    .val()
                    .replace(/,/g, '');

                $('#modalTambahBonKasMerah #jumlah').val(jumlah);
            ...
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'Mode kirim tidak diketahui.'
            });
        ...
        // =====================================================
        // BON KAS SUDAH ADA (DARI LIST)
        // =====================================================
        console.count('AJAX KIRIM');
        $.ajax({
    ...
    $('#btnTambahBonKas').on('click', function () {

        Swal.fire({
            title: 'Tambah Bon Kas',
            html: `
                <div class="text-start">
                    ...
                    <div class="form-check">
                        ...
                        <label class="form-check-label" for="jenisMerah">
                            Bon Kas Merah
                        </label>
                    </div>
                    <div class="form-check mb-3">
                        ...
                        <label class="form-check-label" for="jenisPutih">
                            Bon Kas Putih
                        </label>
                    </div>
            ...
            focusConfirm: false,
            preConfirm: () => {
                return $('input[name="jenisBonKas"]:checked').val();
            }
        }).then((result) => {
            ...
            if (result.value === 'P') {
                ...
                bootstrap.Modal.getOrCreateInstance(
                    document.getElementById('modalTambahBonKasPutih')
                ).show();
            } else {
                bootstrap.Modal.getOrCreateInstance(
        ...
        if (data.JenisBonKas === 'P' && Number(data.TotalPenyesuaian) > 0) {
            ...
            Swal.fire({
                ...
                title: title,
                 width: '900px',
                customClass: {
                    popup: 'swal-bonkas-popup'
                },
                html: `
                        <style>
                        .swal2-popup.swal-bonkas-popup{
                    ...
                    <table class="swal-bonkas-table mb-3">
    ...
    $(document).on('click', '.btn-print', function () {
        ...
        setTimeout(function () {
            console.log($('#printDokumentasiSection').css('display'));
            console.log($('#printDokumentasi').html());
            window.print();
