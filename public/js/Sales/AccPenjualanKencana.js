...
$("#table_AccPenjualan tbody").on("click", "tr", function () {
    ...
    fetch("/accPenjualanKencanaTampilData/" + IdTransaksi)
        .then((response) => response.json())
        .then((data) => {
            console.log("Response accPenjualanKencanaTampilData:", data);

            if (!Array.isArray(data) || data.length === 0) {
                console.error(
                    "Data Tmp_Transaksi tidak ditemukan untuk IdTransaksi:",
                    IdTransaksi
                );

                $("#loading-screen").css("display", "none");

                alert("Data transaksi sudah diproses atau tidak ditemukan.");
                return;
            }

            const detail = data[0];

            divisi.value = detail.NamaDivisi ?? "";
            objek.value = detail.NamaObjek ?? "";
            kelompok_utama.value = detail.NamaKelompokUtama ?? "";
            kelompok.value = detail.NamaKelompok ?? "";

            sub_kelompok.value =
                (detail.IdSubKelompok ?? "") +
                " - " +
                (detail.NamaSubKelompok ?? "");

            saldo_primer.value = detail.SaldoPrimer ?? 0;
            saldo_primerSatuan.value = detail.SatuanPrimer ?? "";

            saldo_sekunder.value = detail.SaldoSekunder ?? 0;
            saldo_sekunderSatuan.value = detail.SatuanSekunder ?? "";

            saldo_tritier.value = detail.SaldoTritier ?? 0;
            saldo_tritierSatuan.value = detail.SatuanTritier ?? "";

            id_transaksi.value = detail.IdTransaksi ?? "";
            id_type.value = detail.IdType ?? "";
            nama_barang.value = detail.NamaType ?? "";
            nama_customer.value = detail.NamaCust ?? "";
            no_sp.value = detail.IDSuratPesanan ?? "";
            ...
            fetch("/accPenjualanKencanaTampilBarcode/" + IdType + "/" + KodeBarang)
                .then((response) => response.json())
                .then((data_barcode) => {
                    ...
                    if (data_barcode.length === 0) {
                        ...
                        for (let i = 0; i < data_barcode.length; i++) {
                            let rowData = {
                                ...
                                Qty_Primer:
                                    data_barcode[i].Qty_Primer == null ||
                                    data_barcode[i].Qty_Primer === ""
                                        ? 0
                                        : parseFloat(data_barcode[i].Qty_Primer),

                                Qty_sekunder:
                                    data_barcode[i].Qty_sekunder == null ||
                                    data_barcode[i].Qty_sekunder === ""
                                        ? 0
                                        : parseFloat(data_barcode[i].Qty_sekunder),

                                Qty:
                                    data_barcode[i].Qty == null ||
                                    data_barcode[i].Qty === ""
                                        ? 0
                                        : parseFloat(data_barcode[i].Qty),
...
prosesButton.addEventListener("click", function (event) {
    ...
    const table = $("#table_AccBarcodePenjualan").DataTable();

    // =====================================================
    // 1. AMBIL BARCODE YANG DICENTANG
    // =====================================================
    const selectedRowData = [];

    table.rows().every(function () {
        const rowData = this.data();
        const checkbox = $(rowData.checkbox);
    ...
    console.log("Jumlah barcode dipilih:", selectedRowData.length);
    ...
    // =====================================================
    // 3. AMBIL KODE BARANG DAN NO INDEKS
    // =====================================================
    const kodebarang = selectedRowData[0].expr2;

    const noindeks = selectedRowData.map(function (row) {
        return row.expr3;
    });
    ...
    // =====================================================
    // 4. AMBIL JUMLAH YANG DIKELUARKAN
    // =====================================================
    const primerKeluar =
        parseFloat(saldo_primerDikeluarkan.value) || 0;

    const sekunderKeluar =
        parseFloat(saldo_sekunderDikeluarkan.value) || 0;

    const tritierKeluar =
        parseFloat(saldo_tritierDikeluarkan.value) || 0;

    console.log("Jumlah Dikeluarkan:", {
        primer: primerKeluar,
        sekunder: sekunderKeluar,
        tritier: tritierKeluar,
    });

    // =====================================================
    // 5. AMBIL SALDO GUDANG
    // =====================================================
    const saldoPrimer =
        parseFloat(saldo_primer.value) || 0;

    const saldoSekunder =
        parseFloat(saldo_sekunder.value) || 0;

    const saldoTritier =
        parseFloat(saldo_tritier.value) || 0;

    console.log("Saldo Gudang:", {
        primer: saldoPrimer,
        sekunder: saldoSekunder,
        tritier: saldoTritier,
    });

    // =====================================================
    // 6. VALIDASI JUMLAH BARANG TERHADAP SALDO
    // =====================================================
    if (primerKeluar > saldoPrimer) {
        alert(
            "Jumlah Primer yang dikeluarkan (" +
                primerKeluar +
                ") melebihi saldo gudang (" +
                saldoPrimer +
                ")."
        );
        return;
    ...
    if (sekunderKeluar > saldoSekunder) {
        alert(
            "Jumlah Sekunder yang dikeluarkan (" +
                sekunderKeluar +
                ") melebihi saldo gudang (" +
                saldoSekunder +
                ")."
    ...
    if (tritierKeluar > saldoTritier) {
        alert(
            "Jumlah Tritier yang dikeluarkan (" +
                tritierKeluar +
                ") melebihi saldo gudang (" +
                saldoTritier +
                ")."
    ...
    const minDO =
        parseFloat(min_do.value) || 0;

    const maxDO =
        parseFloat(max_do.value) || 0;

    console.log("Satuan:", {
        primer: satuanPrimer,
        sekunder: satuanSekunder,
        tritier: satuanTritier,
        satuanDO: satuanDO,
    });

    console.log("Min/Max DO:", {
        min: minDO,
        max: maxDO,
    });

    // -----------------------------------------------------
    // Jika Primer adalah satuan DO
    // -----------------------------------------------------
    if (satuanPrimer === satuanDO) {
        if (
            primerKeluar < minDO ||
            primerKeluar > maxDO
        ) {
    ...
    // -----------------------------------------------------
    // Jika Sekunder adalah satuan DO
    // -----------------------------------------------------
    else if (satuanSekunder === satuanDO) {
        if (
            sekunderKeluar < minDO ||
            sekunderKeluar > maxDO
        ) {
            alert(
    ...
    // -----------------------------------------------------
    // Jika Tritier adalah satuan DO
    // -----------------------------------------------------
    else if (satuanTritier === satuanDO) {
        if (
            tritierKeluar < minDO ||
            tritierKeluar > maxDO
        ) {
    ...
    // =====================================================
    if (
        satuanPrimer !== satuanDO &&
        satuanSekunder !== satuanDO &&
        satuanTritier !== satuanDO
    ) {
        const jumlahKonversi =
            parseFloat(jumlah_konversi.value) || 0;
        ...
        if (jumlahKonversi === 0) {
            alert(
                "Jumlah Konversi Harus Lebih Besar '0' !"
            );
            return;
        }

        if (
            jumlahKonversi < minDO ||
            jumlahKonversi > maxDO
        ) {
            alert(
    ...
    if (
        ...
        alert(
            "Jumlah Primer harus 0 karena satuan Primer NULL."
        );
        return;
    }
    ...
    if (
        ...
        alert(
            "Jumlah Sekunder harus 0 karena satuan Sekunder NULL."
        );
        return;
    }
    ...
    if (
        ...
        alert(
            "Jumlah Tritier harus 0 karena satuan Tritier NULL."
        );
        return;
    }

    // =====================================================
    // 10. MINIMAL ADA JUMLAH YANG DIKELUARKAN
    // =====================================================
    if (
        primerKeluar <= 0 &&
        sekunderKeluar <= 0 &&
        ...
        alert(
            "Jumlah barang yang dikeluarkan harus lebih besar dari 0."
        );
        return;
    }

    // =====================================================
    // 11. BUAT INPUT HIDDEN
    // =====================================================

    // Hindari input hidden lama menumpuk
    form_accJualBarcode
        .querySelectorAll(
            'input[data-acc-dynamic="1"]'
        )
    ...
    kodebarangInput.dataset.accDynamic = "1";

    form_accJualBarcode.appendChild(
        kodebarangInput
    );

    // Jumlah Barcode
    const jumlahdicentangInput =
        document.createElement("input");

    jumlahdicentangInput.type = "hidden";
    jumlahdicentangInput.name = "jumlahDicentang";
    jumlahdicentangInput.value =
        selectedRowData.length;
    jumlahdicentangInput.dataset.accDynamic = "1";

    form_accJualBarcode.appendChild(
        jumlahdicentangInput
    );
    ...
    noindeksInput.dataset.accDynamic = "1";
    ...
    // =====================================================
    // 12. DEBUG SEBELUM SUBMIT
    // =====================================================
    console.log(
        "===================================="
    );
    console.log("ACC PENJUALAN - SUBMIT");
    console.log(
        "Form Action:",
        form_accJualBarcode.action
    );
    console.log(
        "Form Method:",
        form_accJualBarcode.method
    );
    console.log("Kode Barang:", kodebarang);
    console.log(
        "Jumlah Barcode:",
    ...
    console.log("Primer:", primerKeluar);
    console.log("Sekunder:", sekunderKeluar);
    console.log("Tritier:", tritierKeluar);
    console.log(
        "===================================="
    );
