jQuery(function ($) {
    ...
    btn_redisplay.addEventListener("click", async function (event) {
        ...
        table_atas = $("#table_atas").DataTable({
            ...
            destroy: true,
            ajax: {
                url: "PemberhentianMesinCL/getData",
                dataType: "json",
                type: "GET",
                data: function (d) {
                    return $.extend({}, d, {
                    ...
                    render: function (data, type, row) {
                        ...
                        let btnWA = `
                            <button class="btn btn-sm btn-success btn-wa" 
                                ...
                                <i class="fa fa-add"></i> Kirim WA
                            </button>
                        `;

                        // Jika sudah pernah kirim WA
                        if (row.time_send_wa !== null && row.time_send_wa !== "") {
                            btnWA = `
                                <button class="btn btn-sm btn-secondary" 
                                    style="width: 100px;"
                                    disabled>
                                    <i class="fa fa-check"></i> Sudah WA
                                </button>
                            `;
                        }

                        if (
                            row.user_selesai !== null &&
                            row.user_selesai !== ""
                        ) {
                            return `
                                <span style="color: green; font-weight: 600;">
                                    Sudah diselesaikan ${row.user_selesai}
                                </span>
                            `;
                        }
                        ...
                        return `
                            ...
                            <button class="btn btn-sm btn-warning btn-koreksi" 
                                style="width: 60px;"
                                data-id="${row.id_pemberhentian}">
    ...
    $('#table_atas').on('click', '.btn-koreksi', function () {
        ...
        $.ajax({
            url: "PemberhentianMesinCL/getDataKoreksi",
            type: "GET",
            data: {
                _token: csrfToken,
                id_pemberhentian: id,
    ...
    btn_batal.addEventListener("click", async function (event) {
        ...
        $("#" + slcLokasi.id).prop("disabled", false);
